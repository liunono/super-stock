...
def lightning_homerun_loop(pool_df, mode="incremental"):
    ...
    if total_count == 0:
        st.warning("⚠️ 股票池是空的，主力無法出餐！"); return
    ...
    log_box = st.status(f"⚡ 閃電補洞中 ({mode})...", expanded=True)
    ...
    while True:
        ...
        # 💎 安全更新進度條
        initial_done = len(done_list)
        progress_val = min(initial_done / total_count, 1.0)
        progress_bar.progress(progress_val)
        status_text.markdown(f"**🚀 掃描進度：`{initial_done}` / `{total_count}` ({progress_val:.1%})**")

        if remaining_pool.empty:
            st.balloons(); status_text.success("🏆 100% 全壘打！"); break
        ...
        with ThreadPoolExecutor(max_workers=2) as ex:
            ...
            for f in as_completed(futures):
                ...
                if data:
                    ...
                    # 💎 碎片即時更新
                    current_val = min((initial_done + batch_done) / total_count, 1.0)
                    progress_bar.progress(current_val)
                    status_text.markdown(f"**🚀 掃描進度：`{initial_done + batch_done}` / `{total_count}` ({current_val:.1%})**")
...
def beauty_style(df):
    ...
    try:
        ...
        if '營收成長' in df.columns: styled = styled.background_gradient(subset=['營營成長' if '營營成長' in df.columns else '營收成長'], cmap='OrRd', low=0.05, high=0.6)
...
# ================= 4. 主介面設計 (V111.0 終極大按鈕完全體) =================
st.set_page_config(page_title="哲哲量化美學戰情室 V111.0", layout="wide")
...
is_closing = now_tw.hour == 13 and now_tw.minute >= 31

st.title("🛡️ 哲哲量化戰情室 V111.0 — 終極大按鈕完全體")
if is_closing: st.warning("🔔 **收盤時間到！建議執行『暴力覆蓋重掃』獲取最終精準籌碼！**")

tab1, tab2, tab3 = st.tabs(["🚀 七大金剛指揮中心", "💼 持倉監控戰報", "🛠️ 後台管理都更"])

with tab1:
    st.markdown("### 🏆 每日行情智慧掃描 (閃電變速模式)")
    c_scan1, c_scan2, c_scan3 = st.columns(3)
    with c_scan1:
    ...
    if st.button("💎 降臨：超級策略 (基金+ROE+營收+趨勢)", use_container_width=True):
        ...
        else: st.error("⚠️ 請先點擊讀取數據或啟動掃描！")

    # 形態與經典策略 (採用 2x3 大按鈕排版，保證表格寬度)
    ...
    # 💎 數據底牌大解密
    if st.button("🔍 揭開底牌：檢視今日所有抓取數據 (排除幽靈股)", use_container_width=True):
        if 'master_df' in st.session_state:
            st.dataframe(beauty_style(st.session_state['master_df']), use_container_width=True)
        else: st.warning("⚠️ 目前無數據，請先讀取或掃描！")
...
st.caption("本系統由哲哲團隊開發。V111.0 終極大按鈕完全體，賺到流湯不要忘了我！")
