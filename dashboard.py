    ...
    with engine.connect() as conn:
        ...
        conn.execute(text("""
            CREATE TABLE IF NOT EXISTS daily_scans (
                ...
                vol BIGINT, avg_vol BIGINT, kd20 FLOAT, kd60 FLOAT, scan_date DATE, PRIMARY KEY (ticker, scan_date)
            );
        """))
        conn.commit()
...
def style_df(df):
    def color_rsi(val):
        if val >= 70: color = '#FFCCCC'
        elif val >= 55: color = '#FFE5E5'
        elif val <= 30: color = '#CCFFCC'
        ...
        try:
            ...
            if data is not None and len(data) >= 60:
                ...
                return {
                    ...
                    "kd20": round(float(close.iloc[-20]), 2), # 20日扣抵值
                    "kd60": round(float(close.iloc[-60]), 2), # 60日扣抵值
...
# ================= 4. 介面設計 =================
st.set_page_config(page_title="哲哲戰情室 V22.0", layout="wide")
st.title("🛡️ 哲哲量化戰情室 V22.0 - 扣三低提款機版")
...
with tab1:
    ...
    with c_btn1:
        if st.button("📡 讀取今日金庫 (秒開)", use_container_width=True):
            ...
            if not df_db.empty:
                df_db.columns = ['代號','名稱','現價','漲跌(%)','SMA5','MA20','MA60','RSI','BBL','BBU','成交量','均量','KD20','KD60','日期']
    ...
    if 'master_df' in st.session_state:
        ...
        btn_cols = st.columns(6) # 增加到 6 個按鈕
        ...
        strats = [
            ...
            ("扣三低提款機", "💰", (m_df['現價'] > m_df['KD20']) & (m_df['現價'] > m_df['KD60']) & (m_df['MA20'] > m_df['MA60']))
        ]
