...
def send_line(title, df):
    ...
    for _, r in df.head(5).iterrows():
        msg += f"✅ {r.get('ticker','')} {r.get('stock_name','')} | 現價:{r.get('price','0')}\n"
...
# ================= 4. 主介面設計 (V153.0) =================
st.set_page_config(page_title="🛡️ 哲哲量子戰情室 Sponsor V153.0", layout="wide")
...
st.title("🛡️ 哲哲量子戰情室 Sponsor V153.0 — 鋼鐵旗艦封神版")

tab1, tab2, tab3 = st.tabs(["🚀 指揮中心", "💼 庫存戰報", "🛠️ 管理中心"])

with tab1:
    st.markdown("### 🏹 數據連動與進度監控")
    ...
    with c1:
        if st.button("🚀 每日行情：暴力重掃+計算"):
            ...
            st.success("✅ 行情覆蓋重掃完成！"); st.rerun()

    with c2:
        if st.button("🔄 補抓漏失：補抓+計算 (省 API)"):
            ...
            if missing.empty: st.info("🎯 數據已全壘打！")
                ...
                with ThreadPoolExecutor(max_workers=8) as exe:
                    ...
                    for i, f in enumerate(as_completed(futures)):
                        pb.progress((i+1)/len(missing)); st_txt.text(f"🔄 補救中: {i+1} / {len(missing)} ({futures[f]})")
                st.success("✅ 補救計算完成！"); st.rerun()

    with c3:
        if st.button("💼 籌碼補完 (基金張數)"):
            pb = st.progress(0); st_txt = st.empty()
            with ThreadPoolExecutor(max_workers=8) as exe:
                # 這裡調用之前的 update_fund 邏輯
                from dashboard import update_fund # 假設函數在同檔
