    ...
    with engine.connect() as conn:
        ...
        conn.execute(text("""
            CREATE TABLE IF NOT EXISTS daily_scans (
                ...
                roe FLOAT DEFAULT 0, rev_growth FLOAT DEFAULT 0.18, fund_count INT DEFAULT 0,
...
def get_base_price(ticker):
    ...
    start = (datetime.now(TW_TZ) - timedelta(days=150)).strftime('%Y-%m-%d')
    try:
        r = requests.get("https://api.finmindtrade.com/api/v4/data", 
                         params={"dataset": "TaiwanStockPrice", "data_id": cid, "start_date": start, "token": FINMIND_TOKEN}, timeout=10).json()
        if r['msg'] == 'success' and r['data']:
            df = pd.DataFrame(r['data']).rename(columns={'close':'Close','Trading_Volume':'Volume'})
            df['Close'] = df['Close'].astype(float)
            return df
    except: return None

def get_monthly_fund(ticker):
    cid = str(ticker).split('.')[0].strip()
    try:
        r = requests.get("https://api.finmindtrade.com/api/v4/data", 
                         params={"dataset": "TaiwanStockHoldingSharesPer", "data_id": cid, "start_date": (datetime.now(TW_TZ)-timedelta(days=30)).strftime('%Y-%m-%d'), "token": FINMIND_TOKEN}, timeout=10).json()
        if r['msg'] == 'success' and r['data']:
            h_df = pd.DataFrame(r['data'])
            return int(h_df['InvestmentTrustHoldingShares'].iloc[-1] / 1000) if 'InvestmentTrustHoldingShares' in h_df.columns else 0
    except: return 0

def get_quarterly_roe(ticker):
    cid = str(ticker).split('.')[0].strip()
    try:
        r = requests.get("https://api.finmindtrade.com/api/v4/data", 
                         params={"dataset": "TaiwanStockFinancialStatements", "data_id": cid, "start_date": (datetime.now(TW_TZ)-timedelta(days=365)).strftime('%Y-%m-%d'), "token": FINMIND_TOKEN}, timeout=10).json()
        if r['msg'] == 'success' and r['data']:
            f_df = pd.DataFrame(r['data'])
...
# ================= 4. UI 介面 (V150.0) =================
st.set_page_config(page_title="🛡️ 哲哲量子戰情室 Sponsor V150.0", layout="wide")
...
st.markdown("""<style>
    ...
    .stTabs [data-baseweb="tab-list"] button [data-testid="stMarkdownContainer"] p { font-size: 1.5rem; font-weight: 900; }
</style>""", unsafe_allow_html=True)

st.title("🛡️ 哲哲量子戰情室 Sponsor V150.0 — 極致效能版")

tab1, tab2, tab3 = st.tabs(["🚀 指揮中心", "💼 庫存戰報", "🛠️ 管理中心"])

with tab1:
    st.markdown("### 🏹 量子數據連動按鈕 (Sponsor 6000 專用)")
    c1, c2, c3, c4 = st.columns(4)
    pool = pd.read_sql("SELECT ticker, stock_name FROM stock_pool", con=engine)
    
    with c1:
        if st.button("🚀 每日行情快速掃描", help="更新今日價格、RSI、均線"):
            log = st.status("⚡ 每日行情突圍中...")
            with ThreadPoolExecutor(max_workers=8) as exe:
                futures = {exe.submit(update_daily_price, r['ticker'], r['stock_name']): r['ticker'] for _, r in pool.iterrows()}
                for f in as_completed(futures):
                    if f.result(): log.write(f"✅ {futures[f]} 價格到位")
            st.rerun()

    with c2:
        # 每月 10 號左右更新最準
        if st.button("💼 每月籌碼補完計劃", help="抓取投信持股張數 (建議每週/每月執行)"):
            log = st.status("🧠 深度籌碼分析中...")
            with ThreadPoolExecutor(max_workers=8) as exe:
                futures = {exe.submit(update_monthly_fund, r['ticker']): r['ticker'] for _, r in pool.iterrows()}
                for f in as_completed(futures):
                    if f.result(): log.write(f"✅ {futures[f]} 籌碼更新")
            st.rerun()

    with c3:
        # 季報公佈日：5/15, 8/14, 11/14, 3/31
        if st.button("💎 每季財報精算", help="更新 ROE (建議每季或更換標的時執行)"):
            log = st.status("📖 季報數據審計中...")
            with ThreadPoolExecutor(max_workers=8) as exe:
                futures = {exe.submit(update_quarterly_roe, r['ticker']): r['ticker'] for _, r in pool.iterrows()}
                for f in as_completed(futures):
                    if f.result(): log.write(f"✅ {futures[f]} ROE 同步")
            st.rerun()

    with c4:
        if st.button("🔥 終極暴力覆蓋重掃"):
            with engine.begin() as conn:
                conn.execute(text("DELETE FROM daily_scans WHERE scan_date = :t"), {"t": datetime.now(TW_TZ).date()})
            st.rerun()
...
st.caption("本系統由哲哲團隊開發。V150.0 Sponsor 6000 方案專用，賺到流湯不要忘了我！")
