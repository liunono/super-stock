...
# ================= 0. 頁面設定 (必須在最前面) =================
st.set_page_config(page_title="🛡️ 哲哲量子戰情室 V178 終極版", layout="wide")
...
# ================= 2. 核心大腦 (精準打擊模組 & LINE Bot 發射器) =================

def send_line_notif(title, df, action_type="買入"):
    """🚨 復原舊版成功發射的 LINE Bot 推播引擎"""
    ...
    for _, r in df.head(8).iterrows():
        t = r.get('股票代號', r.get('ticker', ''))
        n = r.get('名稱', r.get('stock_name', ''))
        p = r.get('現價', r.get('最新股價', r.get('price', '0')))
        
        # 如果是庫存賣股風控，多抓一個獲利率出來顯示
        if action_type == "賣出":
            profit = r.get('總報酬率(%)', 0)
            msg += f"✅ {t} {n} | 現價:{p} | 獲利: {profit:.1f}%\n"
        else:
            msg += f"✅ {t} {n} | 現價:{p}\n"
    ...
    try: 
        ...
        if r.status_code == 200:
            ...
            st.error(f"❌ LINE 推播失敗: {r.text}")
            return False
    except Exception as e: 
        st.error(f"❌ LINE 連線錯誤: {e}")
        return False

def fetch_fm(dataset, ticker, start_days=160):
    """通用 API 抓取模組 (帶 Bearer Token 授權)"""
...
def update_chip_v_quantum(ticker):
    """💎 籌碼量子修正：只抓 'Investment_Trust' (投信)，計算 60 天淨買賣超"""
...
def update_roe_v_brute(ticker):
    """💎 財報暴力精算：鎖定 'IncomeAfterTaxes' / 'Equity'，直接抓最新季報相除"""
...
# ================= 3. UI 介面設計 =================
st.title("🛡️ 哲哲量子戰情室 Sponsor V178 — 終極推播版")
...
with tab1:
    ...
    if 'master_df' in st.session_state:
        ...
        strategies = {
            "💎 策略 1：超級策略 (投信持股 >= 10張 & ROE > 5%)": (df['fund_count'] >= 10) & (df['roe'] > 0.05),
            "📈 策略 2：帶量突破前高 (股價 > 20日高 & 成交量 > 均量 1.5倍)": (df['現價'] > df['high_20']) & (df['vol'] > df['vol_20'] * 1.5),
            "🚀 策略 3：三線合一多頭 (SMA5 > MA20 > MA60)": (df['sma5'] > df['ma20']) & (df['ma20'] > df['ma60']),
            "🌀 策略 4：布林縮口突破 (股價 > 上軌 & 帶寬 < 0.2)": (df['現價'] > df['bbu']) & (df['bb_width'] < 0.2) & (df['bb_width'] > 0),
            "👑 策略 5：九成勝率 ATM (股價 > 20日前 & 成交量 > 均量 1.2倍)": (df['現價'] > df['kd20']) & (df['vol'] > df['vol_20'] * 1.2),
            "🛡️ 策略 6：低階抄底 (RSI < 40 & 站回五日線)": (df['rsi'] < 40) & (df['rsi'] > 0) & (df['現價'] > df['sma5']),
            "🎯 策略 7：強勢回測支撐 (股價接近 MA20 2%以內)": (abs(df['現價'] - df['ma20']) / df['ma20'] < 0.02) & (df['現價'] > 0)
        }
        ...
        for title, condition in strategies.items():
            if st.button(title):
                ...
                if res.empty: 
                    ...
                    if enable_line_buy:
                        if send_line_notif(title.split('(')[0].strip(), res, "買入"):
            ...
            if st.button("📲 將目前【觸發之風控警報】發送至 LINE"):
                ...
                for s_title, s_cond in sell_strats.items():
                    ...
                    if not alert_df.empty:
                        has_alert = True
                        send_line_notif(f"警報: {s_title.split('(')[0].strip()}", alert_df, "賣出")
    ...
    with colB:
        ...
        if pool_file:
            ...
            if 'ticker' in df.columns and 'stock_name' in df.columns:
                ...
                df = df.drop_duplicates(subset=['ticker']) # 自動去重
                with engine.begin() as conn:
                ...
                st.error("❌ CSV 格式錯誤，請確保首列包含 ticker, stock_name 欄位。")
        ...
        if port_file and st.button("執行上傳庫存"):
            ...
            if all(c in df.columns for c in ['ticker', 'stock_name', 'entry_price', 'qty']):
                ...
                st.error("❌ CSV 格式錯誤，請確保首列包含 ticker, stock_name, entry_price, qty 欄位。")
