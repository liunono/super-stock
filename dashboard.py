...
# ================= 4. 介面設計 (V56.0 巔峰避險完全體) =================
st.set_page_config(page_title="哲哲戰情室 V56.0", layout="wide")
st.title("🛡️ 哲哲量化戰情室 V56.0 — 終極避險提款機")

tab1, tab2, tab3 = st.tabs(["🚀 核心買股策略掃描", "💼 資產獲利 & 賣出策略", "🛠️ 後台管理中心"])

# --- Tab 1: 買股策略 (徹底避開 % 報錯) ---
with tab1:
    st.markdown("### 🏆 每日全市場掃描 (避險轉義版)")
    c1, c2 = st.columns(2)
    with c1:
        if st.button("📡 讀取今日快取數據", use_container_width=True):
            # 💎 戰術變換：SQL 不使用百分比符號，讀出後再改名，避開 ValueError
            query = text("""
                SELECT ticker as 代號, stock_name as 名稱, price as 現價, 
                change_pct as `漲跌_pct`, sma5, ma20, rsi as RSI, bbu, vol, avg_vol, kd20, kd60 
...
with tab2:
    ...
    if not df_p.empty:
        if st.button("🔄 更新即時獲利 (強力逐檔掃描)", use_container_width=True):
            with st.spinner("連線交易所，拒絕數據漏抓..."):
                p_map = {}
                for t in df_p['ticker'].tolist():
                    try: # 逐檔下載，確保 None 值 100% 回歸
                        prices = yf.download(t, period="1d", interval="1m", progress=False)['Close']
                        p_map[t] = prices.iloc[-1]
                    except:
                        try: # 切換市場尾綴
        ...
        if 'rt_p' in st.session_state:
            ...
            st.metric("當前實質總獲利", f"${df_p['獲利'].sum():,.0f}")
        ...
        for i, (name, icon) in enumerate(s_btns):
            if m_cols[i].button(f"{icon} {name}", use_container_width=True):
                if 'master_df' in st.session_state and 'rt_p' in st.session_state:
                    ...
                    else: st.success(f"✅ 持倉安全，未觸發『{name}』。")
                else: st.warning("💡 請先讀取行情數據並更新現價。")
    else: st.info("持倉為空。")
