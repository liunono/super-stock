...
def process_logic(ticker, name, df):
    try:
        ...
        return {
            ...
            "bb_width": float((bb.iloc[-1, 2] - bb.iloc[-1, 0]) / ta.sma(c, 20).iloc[-1]),
...
def quantum_diagnose_loop(pool_df, mode="incremental"):
    ...
    for i in range(0, len(tickers), batch_size):
        ...
        try:
            data_all = yf.download(curr_batch, period="7mo", group_by='ticker', threads=True, progress=False)
            for t in curr_batch:
                df_s = data_all[t] if len(curr_batch) > 1 else data_all
                res, msg = process_logic(t, names[t], df_s)
                # 自動校準 .TW/.TWO
                if res is None and ".TW" in t:
                    df_alt = yf.download(t.replace(".TW", ".TWO"), period="7mo", progress=False)
            ...
            p_bar.progress(min(max(current_done / total, 0.0), 1.0))
            time.sleep(3)
...
# ================= 4. 主介面設計 (V125.0 完全體) =================
st.set_page_config(page_title="哲哲量化戰情室 V125.0", layout="wide")
st.markdown("""<style>
    div.stButton > button { height: 4.5em; font-size: 1.3rem !important; font-weight: bold !important; border-radius: 15px; width: 100% !important; margin-bottom: 12px; }
    .big-font { font-size:45px !important; font-weight: bold; color: #FF3333; }
</style>""", unsafe_allow_html=True)

st.title("🛡️ 哲哲量化戰情室 V125.0 — 終極鋼鐵完全體")

tab1, tab2, tab3 = st.tabs(["🚀 七大金剛指揮中心", "💼 資產即時戰報", "🛠️ 後台管理與汰除"])

with tab1:
    st.markdown("### 🏆 每日行情智慧全掃描")
    ...
    with c2:
        if st.button("⚡ 啟動增量量子掃描"):
            pool = pd.read_sql("SELECT ticker, stock_name FROM stock_pool", con=engine)
            quantum_diagnose_loop(pool, mode="incremental"); st.rerun()
    with c3:
        if st.button("🔥 暴力覆蓋重掃"):
            pool = pd.read_sql("SELECT ticker, stock_name FROM stock_pool", con=engine)
            quantum_diagnose_loop(pool, mode="reset"); st.rerun()
    ...
    st.divider()
    if st.button("🔍 揭開底牌：數據照妖鏡 (檢視 350 檔原始數據)"):
        if 'master_df' in st.session_state:
...
with tab2:
    ...
    if not df_p.empty:
        ...
        p_prices = pd.read_sql(text("SELECT ticker, price, sma5, ma20, rsi FROM daily_scans WHERE scan_date = :t AND price > 0.5"), con=engine, params={"t": datetime.now(TW_TZ).date()})
        ...
        if m_s1.button("💀 均線死叉"): st.write(df_p[df_p['sma5'] < df_p['ma20']])
        if m_s2.button("🔥 RSI 過熱"): st.write(df_p[df_p['rsi'] > 80])
        if m_s3.button("💰 利潤止盈"): st.write(df_p[df_p['報酬率(%)'] > 20])
        if m_s4.button("📉 破位停損"): st.write(df_p[df_p['報酬率(%)'] < -10])
        if m_s5.button("⚠️ 跌破月線"): st.write(df_p[df_p['price'] < df_p['ma20']])

with tab3:
    st.subheader("🛠️ 數據管理與「批量汰除」中心")
    # 💎 批量刪除功能
    st.markdown("#### 🚫 批量剔除股票 (上櫃代號或髒數據)")
    del_list = st.text_area("請輸入要刪除的代號 (例如: 3293.TW, 3491.TW)，用逗號或換行隔開")
    if st.button("🔥 鋼鐵汰除黑名單"):
        if del_list:
            tickers_to_del = [t.strip().upper() for t in del_list.replace('\n', ',').split(',') if t.strip()]
            with engine.begin() as conn:
            ...
            st.success(f"✅ 已將 {len(tickers_to_del)} 檔標的從系統中徹底抹除！")
    
    st.divider()
    f1 = st.file_uploader("上傳股票池 CSV (ticker, stock_name, sector, fund_count)")
...
st.caption("本系統由哲哲團隊開發。V125.0 終極完全體，賺到流湯不要忘了我！")
