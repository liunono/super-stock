    ...
    while True:
        ...
        with ThreadPoolExecutor(max_workers=2) as ex:
            ...
            for f in as_completed(futures):
                ...
                counter += 1
                # 💎 變速節奏：每 15 檔進入安全潛行
                if counter % 15 == 0: time.sleep(random.uniform(2.0, 3.0))
...
# ================= 4. 主介面設計 (V106.0 完全體) =================
st.set_page_config(page_title="哲哲量化美學戰情室 V106.0", layout="wide")
...
st.title("🛡️ 哲哲量化戰情室 V106.0 — 暴力美學旗艦版")

tab1, tab2, tab3 = st.tabs(["🚀 七大金剛指揮中心", "💼 持倉即時監控", "🛠️ 後台數據都更"])

# --- Tab 1: 核心掃描 ---
with tab1:
    st.markdown("### 🏆 每日智慧全掃描 (閃電碎分模式)")
    ...
    if st.button("💎 降臨：超級策略 (基金+ROE+營收+趨勢)", use_container_width=True):
        ...
        else: st.warning("請先讀取數據！")

    # 形態策略
    st.markdown("#### 🔹 形態還原 (圖一、二、三)")
...
st.caption("本系統由哲哲團隊開發。V106.0 暴力美學版，賺到流湯不要忘了我！")
