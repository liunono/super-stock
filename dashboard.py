...
try:
    ...
    with engine.connect() as conn:
        ...
        # B. 🔥 強制都更：檢查 portfolio 欄位，缺什麼補什麼！解決 DatabaseError 的核心！
...
def process_ocr_v33(files):
    """終極 OCR：解決數據錯位與排除負數損益"""
...
# ================= 4. 介面設計 (V33.0 巔峰版) =================
st.set_page_config(page_title="哲哲戰情室 V33.0", layout="wide")
st.title("🛡️ 哲哲量化戰情室 V33.0 — 終極封神提款機")
...
with tab3:
    ...
    if ups and st.button("🚀 啟動 AI 校準辨識", use_container_width=True):
        with st.spinner("智慧校準中..."):
            df_ocr = process_ocr_v33(ups)
            if not df_ocr.empty:
                # 💎 防錯：強制轉數值並清洗 NaN，解決 DatabaseError 的核心！
                ...
                try:
                    with engine.begin() as conn:
                        # 只選取正確欄位寫入
                        final_to_sql = df_ocr[['ticker', 'stock_name', 'entry_price', 'qty']]
                        final_to_sql.to_sql('portfolio', con=conn, if_exists='append', index=False)
                    st.success("✅ 數據校準寫入完成！")
                    st.dataframe(final_to_sql)
                except Exception as db_e:
                    st.error(f"❌ 資料庫寫入失敗！原因：{db_e}")

st.caption("本系統由哲哲冠軍團隊開發，數字會說話！進出請參考報價，投資有風險！")
