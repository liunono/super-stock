...
try:
    ...
    with engine.connect() as conn:
        ...
        # B. 🔥 強力都更：檢查並補齊缺失欄位，解決 DatabaseError
        # 檢查 portfolio
...
st.markdown("""<style>.big-font { font-size:42px !important; font-weight: bold; color: #FF3333; }</style>""", unsafe_allow_html=True)

# ================= 4. 主介面設計 (V66.0 鋼鐵版) =================
st.set_page_config(page_title="哲哲戰情室 V66.0", layout="wide")
st.title("🛡️ 哲哲量化戰情室 V66.0 — 鋼鐵地基與全能完全體")

tab1, tab2, tab3 = st.tabs(["🚀 買股策略掃描", "💼 資產獲利 & 賣出策略", "🛠️ 後台管理中心"])

# --- Tab 1: 買股策略 ---
with tab1:
    st.markdown("### 🏆 每日行情掃描中心 (後台存取)")
    c1, c2 = st.columns(2)
    with c1:
    ...
    with c2:
        if st.button("⚡ 啟動渦輪並行掃描 (全股票池)", use_container_width=True):
            ...
            if not pool.empty:
                ...
                status_scan = st.status("🚀 啟動並行掃描...", expanded=False)
    ...
    if 'master_df' in st.session_state and not st.session_state['master_df'].empty:
        ...
        for i, (name, icon, mask) in enumerate(strats):
            if cols[i].button(f"{icon} {name}", use_container_width=True):
                ...
                send_line_report(name, res, icon) # 💎 LINE 歸位
...
with tab2:
    ...
    if not df_p.empty:
        if st.button("🔄 更新即時獲利與存檔 (開啟強攻日誌)", use_container_width=True):
            ...
            st.session_state['rt_p_v66'] = p_map
        ...
        if 'rt_p_v66' in st.session_state:
            df_p['現價'] = df_p['ticker'].map(st.session_state['rt_p_v66'])
            ...
            if m_cols[i].button(f"{icon} {name}", use_container_width=True):
                ...
                if not scans_df.empty:
                    ...
                    if not res.empty:
                        ...
                        st.dataframe(style_df(res[['stock_name_x', 'ticker', '現價_y', '報酬率(%)']].rename(columns={'現價_y':'現價'})))
                        send_line_report(f"賣訊：{name}", res, icon) # 💎 LINE 噴發
