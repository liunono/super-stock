...
def update_daily_price(ticker, name):
    ...
    with engine.begin() as conn:
        conn.execute(text("""INSERT INTO daily_scans (ticker, stock_name, price, change_pct, sma5, ma20, ma60, rsi, bbl, bbu, vol, avg_vol, scan_date, kd20, kd60, high_20, vol_20, bb_width) 
            ...
            ON DUPLICATE KEY UPDATE price=VALUES(price), change_pct=VALUES(change_pct), sma5=VALUES(sma5), ma20=VALUES(ma20), ma60=VALUES(ma60), rsi=VALUES(rsi), vol=VALUES(vol), avg_vol=VALUES(avg_vol)"""), data)
    return True

def update_monthly_fund(ticker):
...
def update_quarterly_roe(ticker):
...
# ================= 3. 主介面設計 (V155.0) =================
st.set_page_config(page_title="🛡️ 哲哲量子戰情室 Sponsor V155.0", layout="wide")

st.markdown("""<style> [data-testid="stBaseButton-secondary"] { width: 100% !important; height: 3.5em !important; font-size: 1.15rem !important; font-weight: 800 !important; border-radius: 12px !important; margin-bottom: 8px !important; background: linear-gradient(135deg, #FF3333 0%, #AA0000 100%) !important; color: white !important; } .big-font { font-size:60px !important; font-weight: 900; color: #FF3333; text-shadow: 2px 2px 4px #ddd; } </style>""", unsafe_allow_html=True)

st.title("🛡️ 哲哲量子戰情室 Sponsor V155.0 — 終極原子體版")

tab1, tab2, tab3 = st.tabs(["🚀 指揮中心", "💼 庫存戰報", "🛠️ 管理中心"])

with tab1:
    st.markdown("### 🏹 週期分流與暴力計算 (Sponsor 6000)")
    ...
    with c1:
        if st.button("🚀 每日行情：暴力重掃", key="btn_daily"):
            ...
            with ThreadPoolExecutor(max_workers=8) as exe:
                futures = {exe.submit(update_daily_price, r['ticker'], r['stock_name']): r['ticker'] for _, r in pool.iterrows()}
                for i, f in enumerate(as_completed(futures)):
                    pb.progress((i+1)/total); st_txt.text(f"🚀 行情: {i+1}/{total} ({futures[f]})")
            st.rerun()
    with c2:
        if st.button("🔄 補救掃描：補漏掉標的", key="btn_catchup"):
            ...
            if missing.empty: st.info("🎯 今日行情已全壘打！")
            else:
                pb = st.progress(0); st_txt = st.empty()
                with ThreadPoolExecutor(max_workers=8) as exe:
                    futures = {exe.submit(update_daily_price, r['ticker'], r['stock_name']): r['ticker'] for _, r in missing.iterrows()}
                    for i, f in enumerate(as_completed(futures)):
                        pb.progress((i+1)/len(missing)); st_txt.text(f"🔄 補抓: {i+1}/{len(missing)}")
                st.rerun()
    with c3:
        if st.button("💼 每月/週：投信籌碼", key="btn_monthly"):
            pb = st.progress(0); st_txt = st.empty()
            with ThreadPoolExecutor(max_workers=8) as exe:
                futures = {exe.submit(update_monthly_fund, r['ticker']): r['ticker'] for _, r in pool.iterrows()}
                for i, f in enumerate(as_completed(futures)):
                    pb.progress((i+1)/total); st_txt.text(f"💼 籌碼: {i+1}/{total}")
            st.rerun()
    with c4:
        if st.button("💎 每季：財報 ROE", key="btn_quarterly"):
            pb = st.progress(0); st_txt = st.empty()
            with ThreadPoolExecutor(max_workers=8) as exe:
                futures = {exe.submit(update_quarterly_roe, r['ticker']): r['ticker'] for _, r in pool.iterrows()}
                for i, f in enumerate(as_completed(futures)):
                    pb.progress((i+1)/total); st_txt.text(f"💎 財報: {i+1}/{total}")
            st.rerun()
    with c5:
        if st.button("🔥 覆蓋重掃 (清空今日)", key="btn_clear"):
    ...
    cr, cm = st.columns(2)
    with cr:
        if st.button("📡 讀取今日快取數據", key="btn_read"):
    ...
    with cm:
        if st.button("🔍 數據照妖鏡 (檢視全庫)", key="btn_mirror"):
    ...
    if 'master_df' in st.session_state:
        ...
        for name, cond in s_list:
            if st.button(name, key=f"buy_{name}"):
                res = df[eval(cond)]; st.dataframe(beauty_style(res), width=1500); send_line(f"買訊：{name}", res)
...
with tab2:
    ...
    if not df_p.empty:
        # 💎 修正：新增庫存更新按鈕
        if st.button("🔄 庫存資產即時更新與精算", key="btn_update_port"):
            pb_p = st.progress(0)
            with ThreadPoolExecutor(max_workers=8) as exe:
                futures = {exe.submit(update_daily_price, r['ticker'], r['stock_name']): r['ticker'] for _, r in df_p.iterrows()}
                for i, f in enumerate(as_completed(futures)):
...
with tab3:
    ...
    with c_p:
        f1 = st.file_uploader("股票池 CSV", type="csv", key="up_pool")
        if f1 and st.button("💾 儲存股票池", key="save_pool"):
            ...
            st.success("更新成功")
    with c_c:
        f2 = st.file_uploader("持倉 CSV", type="csv", key="up_port")
        if f2 and st.button("💾 儲存持倉", key="save_port"):
...
st.caption("本系統由哲哲團隊開發。V155.0 Sponsor 終極原子體版，贏到流湯不是夢！")
