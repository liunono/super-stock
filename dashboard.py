...
def calc_and_save_full(ticker, name):
    ...
    with engine.begin() as conn:
        conn.execute(text("""INSERT INTO daily_scans (ticker, stock_name, price, change_pct, sma5, ma20, ma60, rsi, bbl, bbu, vol, avg_vol, scan_date, kd20, kd60, high_20, vol_20, bb_width) 
            ...
            ON DUPLICATE KEY UPDATE price=VALUES(price), change_pct=VALUES(change_pct), sma5=VALUES(sma5), ma20=VALUES(ma20), rsi=VALUES(rsi), vol=VALUES(vol), bb_width=VALUES(bb_width), high_20=VALUES(high_20)"""), data)
    return True

def update_chip_v2(ticker):
    df = fetch_fm("TaiwanStockHoldingSharesPer", ticker, 35)
...
def update_roe_v2(ticker):
    df = fetch_fm("TaiwanStockFinancialStatements", ticker, 365)
    roe = 0
    if df is not None:
        r_row = df[df['type'] == 'ReturnOnEquityAftTax']
        roe = float(r_row['value'].iloc[-1])/100 if not r_row.empty else 0
    with engine.begin() as conn:
...
# ================= 3. UI 介面設計 (V162.0) =================
st.set_page_config(page_title="🛡️ 哲哲量子戰情室 Sponsor V162.0", layout="wide")
...
st.title("🛡️ 哲哲量子戰情室 Sponsor V162.0 — 量子終極版")
...
with tab1:
    ...
    with c1:
        if st.button("🚀 每日行情：暴力重掃", key="b_d"):
            ...
            st.rerun()
    with c2:
        if st.button("🔄 補救掃描：補抓+計算", key="b_f"):
            ...
            if missing.empty: st.session_state['status_msg'] = "🎯 數據已全壘打！"
                ...
                st.rerun()
    with c3:
        if st.button("💼 籌碼補完：投信張數", key="b_c"):
            pb = st.progress(0); st_txt = st.empty(); s = 0
            with ThreadPoolExecutor(max_workers=10) as exe:
                futures = {exe.submit(update_chip_v2, r['ticker']): r['ticker'] for _, r in pool.iterrows()}
                for i, fut in enumerate(as_completed(futures)):
                    if fut.result(): s += 1
            ...
            st.rerun()
    with c4:
        if st.button("💎 財報精算：ROE 同步", key="b_r"):
            pb = st.progress(0); st_txt = st.empty(); s = 0
            with ThreadPoolExecutor(max_workers=10) as exe:
                futures = {exe.submit(update_roe_v2, r['ticker']): r['ticker'] for _, r in pool.iterrows()}
                for i, fut in enumerate(as_completed(futures)):
                    if fut.result(): s += 1
    ...
    with cm:
        if st.button("🔍 數據照妖鏡 (紅字報錯)", key="btn_mir"):
    ...
    with cd:
        # 💎 哲哲新增：數據診斷下載
        st.markdown("##### 🧪 診斷：下載 ROE 與 籌碼")
        diag_df = pd.read_sql(text("SELECT ticker, stock_name, roe, fund_count FROM daily_scans WHERE scan_date = :t"), con=engine, params={"t": today})
        st.download_button("📥 下載目前 ROE/籌碼清單", data=diag_df.to_csv(index=False).encode('utf-8-sig'), file_name=f"diag_{today}.csv")
...
with tab2:
    ...
    if not df_p.empty:
        if st.button("🔄 同步庫存行情+精算損益", key="btn_u_p"):
            pb_p = st.progress(0)
            with ThreadPoolExecutor(max_workers=10) as exe:
        ...
        for j, (s_name, s_cond) in enumerate(sell_btns):
            if m_c[j].button(s_name, key=f"sel_{s_name}"):
                res_sell = df_display[eval(s_cond)]; send_line(f"賣訊：{s_name}", res_sell); st.success(f"✅ 已通報！")
...
st.caption("本系統由哲哲團隊開發。V162.0 Sponsor 全能旗艦版，贏到流湯不要忘了我！")
