...
# ================= 1. 系統地基 (強制解碼與自動化都更) =================
try:
    # 💎 關鍵修復：加入 charset=utf8mb4 解決 ???? 亂碼問題
    ...
    with engine.connect() as conn:
        # A. 確保三大核心表格地基穩固
        conn.execute(text("SET NAMES utf8mb4;")) # 強制資料庫端解碼
...
# ================= 4. 介面設計 (V48.0 巔峰版) =================
st.set_page_config(page_title="哲哲戰情室 V48.0", layout="wide")
st.title("🛡️ 哲哲量化戰情室 V48.0 — 終極全能提款機")

tab1, tab2, tab3 = st.tabs(["🚀 核心買股策略", "💼 持倉獲利監控", "🛠️ 後台管理中心"])
    ...
    if 'master_df' in st.session_state:
        ...
        for i, (name, icon, mask) in enumerate(strats):
            if cols[i].button(f"{icon} {name}", use_container_width=True):
                ...
                st.write(f"符合『{name}』標的：")
                st.dataframe(style_df(res))
                send_line_report(name, res, icon)

# --- Tab 2: 資產獲利監控 (解決中文字亂碼顯示) ---
with tab2:
    st.header("💼 我的資產亮牌區")
    # 💎 修正：透過 LEFT JOIN 從股票池抓名稱，防止亂碼
    df_p = pd.read_sql("""
    ...
    else: st.info("目前持倉為空，請至 Tab 3 上傳。")
...
with tab3:
    ...
    with col1:
        ...
        st.download_button("📥 下載股票池範本 CSV", sample_pool.to_csv(index=False).encode('utf-8-sig'), "sample_pool.csv", "text/csv")
        
        f_pool = st.file_uploader("上傳股票池", type="csv", key="f1")
        if f_pool and st.button("💾 匯入股票池"):
            try:
                # 💎 讀取時強制 utf-8-sig 防止亂碼
                df_pool_up = pd.read_csv(f_pool, encoding='utf-8-sig')
                df_pool_up.to_sql('stock_pool', con=engine, if_exists='append', index=False)
                st.success("匯入成功！")
            except Exception as e: st.error(f"匯入失敗 (可能是代號重複)：{e}")

    with col2:
        st.subheader("💰 持倉管理 (Portfolio)")
        # 💎 範例下載功能
        sample_port = pd.DataFrame({'ticker': ['2330.TW'], 'stock_name': ['台積電'], 'entry_price': [700.0], 'qty': [1000]})
        st.download_button("📥 下載持倉範本 CSV", sample_port.to_csv(index=False).encode('utf-8-sig'), "sample_portfolio.csv", "text/csv")
        
        f_port = st.file_uploader("上傳持倉數據", type="csv", key="f2")
        if f_port and st.button("💾 存入持倉"):
            try:
                df_up = pd.read_csv(f_port, encoding='utf-8-sig')
                with engine.begin() as conn:
                    # 💎 解決重複寫入報錯：先刪除 CSV 內的現有代號
...
st.caption("本系統由哲哲團隊開發。數字會說話，賺到流湯不要忘了我！")
