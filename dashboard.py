...
# ================= 2. 核心大腦 (數據分流與 0 元防護引擎) =================

def fetch_full_stock_package(ticker, name):
    """💎 哲哲核武抓取：嚴格封殺 0 元數據，數據分流存檔"""
    try:
        # ⚡ 閃電節奏：0.5s - 0.8s
...
def lightning_homerun_loop(pool_df, mode="incremental"):
    ...
    while True:
        ...
        p_text.markdown(f"**🚀 實際成功入庫：`{curr_done}` / `{total_count}` ({curr_done/total_count:.1%})**")

        if remaining_pool.empty:
            st.balloons(); p_text.success(f"🏆 今日數據全數滿血歸位！"); break
        ...
        with ThreadPoolExecutor(max_workers=2) as ex:
            ...
            for f in as_completed(futures):
                ...
                data, err = f.result()
                batch_done_count += 1
                if batch_done_count % 15 == 0: time.sleep(2.0) # 呼吸
                
                if data:
                    # 💎 強力 UPSERT：先刪後塞，保證資料庫不打架
                    ...
                    # UI 即時跳動
                    dynamic_val = min((curr_done + batch_done_count) / total_count, 1.0)
                    p_bar.progress(dynamic_val)
                    p_text.markdown(f"**🚀 實際成功入庫：`{curr_done + batch_done_count}` / `{total_count}` ({dynamic_val:.1%})**")
                    log_box.write(f"✅ 真錢入庫：{data['stock_name']} (${data['price']})")
                else:
                    fail_tracker[ticker] = fail_tracker.get(ticker, 0) + 1
                    if fail_tracker[ticker] >= 3:
                        # 三振出局：存入 0.01 防止無限迴圈，但會被 UI 濾掉
                        pd.DataFrame([{"ticker": ticker, "stock_name": "無效標的", "scan_date": today, "price": 0.01}]).to_sql('daily_scans', con=engine, if_exists='append', index=False)
                        log_box.write(f"🚫 {ticker} 判定缺失")
                    else:
                        log_box.write(f"⚠️ {ticker} 暫跳：{err}")
        round_num += 1; time.sleep(5)
    log_box.update(label="✨ 任務結束，請點擊『讀取今日數據』。", state="complete")
...
# ================= 4. 主介面設計 (V116.0 超級大按鈕版) =================
st.set_page_config(page_title="哲哲量化美學戰情室 V116.0", layout="wide")
st.markdown("""<style>
    .big-font { font-size:48px !important; font-weight: bold; color: #FF3333; text-shadow: 2px 2px 4px #eee; }
    div.stButton > button { height: 4em; font-size: 1.4rem !important; font-weight: bold !important; border-radius: 15px; margin-bottom: 12px; transition: 0.3s; width: 100% !important; }
    div.stButton > button:hover { border-color: #FF3333; color: #FF3333; background-color: #FFF5F5; }
</style>""", unsafe_allow_html=True)

now_tw = datetime.now(TW_TZ)
st.title("🛡️ 哲哲量化戰情室 V116.0 — 數據重生完全體")

# 🕒 收盤同步警告
if now_tw.hour == 13 and now_tw.minute >= 31:
    st.warning("🔔 **現在 1:31 PM 收盤時間，強烈建議『暴力覆蓋重掃』以確保決策數據 100% 正確！**")

tab1, tab2, tab3 = st.tabs(["🚀 指揮官指揮中心", "💼 資產即時監控", "🛠️ 後台數據都更"])

# --- Tab 1: 核心掃描 ---
with tab1:
    st.markdown("### 🏆 每日行情智慧全掃描 (強力 UPSERT)")
    ...
    st.divider()
    if st.button("🔍 揭開底牌：檢視今日 350 檔所有數據 (不限股價)", use_container_width=True):
        if 'master_df' in st.session_state:
            st.dataframe(beauty_style(st.session_state['master_df']), use_container_width=True)
        else: st.warning("⚠️ 目前無快取數據。")
...
with tab2:
    ...
    if not df_p.empty:
        if st.button("🔄 更新持倉現價 (封殺 0 元數據)", use_container_width=True):
        ...
        for i, (name, icon) in enumerate(s_btns):
            if m_sell[i].button(f"{icon} {name}", use_container_width=True):
                masks = [display_p['sma5'] < display_p['ma20'], display_p['rsi'] > 80, display_p['報酬率(%)'] > 20, display_p['報酬率(%)'] < -10, display_p['price'] < display_p['ma20']]
                res_s = display_p[masks[i]].copy()
                if not res_s.empty:
...
st.caption("本系統由哲哲團隊開發。V116.0 數據重生完全體，賺到流湯不要忘了我！")
