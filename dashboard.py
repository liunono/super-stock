...
def fetch_full_stock_package(ticker, name):
    ...
    try:
        ...
        if d.empty or len(d) < 65:
            ...
            if d.empty or len(d) < 65:
                return None, f"數據不足或已被 Yahoo 封鎖"
        ...
        res_dict = {
            ...
            "fund_count": int(np.random.randint(50, 150)) # 暫用模擬，避開額外請求
        }
        return res_dict, None
    except Exception as e:
        err_msg = str(e)
        if "Too Many Requests" in err_msg:
            time.sleep(5) # 遇到封鎖自動休息 5 秒
        return None, err_msg
...
st.markdown("""<style>.big-font { font-size:48px !important; font-weight: bold; color: #FF3333; }</style>""", unsafe_allow_html=True)

# ================= 4. 主介面設計 (V83.0 智慧避雷版) =================
st.set_page_config(page_title="哲哲戰情室 V83.0", layout="wide")
st.title("🛡️ 哲哲量化戰情室 V83.0 — 智慧避雷與數據滿血版")

tab1, tab2, tab3 = st.tabs(["🚀 核心策略掃描", "💼 資產即時戰報", "🛠️ 後台管理中心"])

# --- Tab 1: 買股策略 ---
with tab1:
    st.markdown("### 🏆 每日行情智慧掃描 (防封鎖模式)")
    c1, c2 = st.columns(2)
    with c1:
        if st.button("📡 讀取今日快取數據", use_container_width=True):
            ...
            else: st.warning("今日無數據，請點擊右側啟動掃描。")
            
    with c2:
        if st.button("⚡ 啟動並行渦輪掃描 (避開警衛版)", use_container_width=True):
            pool = pd.read_sql("SELECT ticker, stock_name FROM stock_pool", con=engine)
            if not pool.empty:
                res, errors, prog = [], [], st.progress(0)
                status_scan = st.status("🚀 正在繞過數位警衛，啟動智慧抓取...", expanded=True)
                
                # 💎 關鍵：降低並行數量 (max_workers=5)，並在函數內加入 delay
                with ThreadPoolExecutor(max_workers=5) as ex:
                    ...
                    for count, f in enumerate(as_completed(futures)):
                        ...
                        if data:
                            ...
                            errors.append(f"{futures[f]}: {err}")
                            status_scan.write(f"⚠️ {futures[f]} 暫時跳過：{err}")
                ...
                if res:
                    ...
                    if errors:
                        with st.expander("🔍 查看失敗清單 (可能是 Yahoo 暫時封鎖)"):
...
with tab2:
    ...
    if not df_p.empty:
        if st.button("🔄 更新即時獲利 (避雷強攻版)", use_container_width=True):
            p_map, all_res = {}, []
            status_box = st.status("🚀 正在執行持倉數據同步 (智慧冷卻中)...", expanded=True)
            for idx, t in enumerate(df_p['ticker'].tolist()):
                ...
                else: status_box.write(f"❌ {t} 暫時跳過：{err}")
                time.sleep(1) # 持倉更新稍微放慢，保證成功率
            if all_res:
            ...
            st.session_state['rt_p_v83'] = p_map
            st.rerun()

        if 'rt_p_v83' in st.session_state:
            df_p['現價'] = df_p['ticker'].map(st.session_state['rt_p_v83'])
        ...
        st.divider()
        st.markdown("### 🎯 五大必勝賣股決策 (LINE 全線歸位)")
        m_cols = st.columns(5)
...
st.caption("本系統由哲哲團隊開發。智慧避雷版，賺到流湯不要忘了我！")
